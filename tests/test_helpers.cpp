...
string test_search(string searchkey, bool build)
{
    ...
    CUBuilding *tmp = CUBuildingNet.searchForBuilding(searchkey);
    if(tmp) {
      cout << "Name of building returned by function: " << tmp->name << endl;
    }
    else {
...
string test_addBuildinginfo(string prev, string name, int num, bool build)
{
    ...
    CUBuildingNet.addBuildingInfo(CUBuildingNet.searchForBuilding(prev),name, num);
...
string test_transmitRoomInfo(string arr[], int len, bool build)
{
    ...
    for(int i=0;i<len;i++)
    {
        CUBuildingNet.transmitRoomInfo(arr[i]);
    }
    string output = testing::internal::GetCapturedStdout();
